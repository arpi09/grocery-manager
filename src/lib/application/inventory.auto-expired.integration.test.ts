...
describe('Inventory auto-expired integration', () => {
	...
	it('bulk-discards auto-expired items for a location', async () => {
		await seed('user-1');
		const today = new Date().toISOString().slice(0, 10);
		await service.createItem(
			householdId,
		...
		const cleared = await service.bulkDiscardAutoExpired(
			householdId,
			'fridge',
			'user-1',
			'owner'
		...
		expect(await service.countAutoExpiredByLocation(householdId, 'fridge')).toBe(0);
		expect(await service.countActiveByLocation(householdId, 'fridge')).toBe(0);
	});

	it('respects household grace days when classifying auto-expired items', async () => {
		await seed('user-1');
		const today = new Date().toISOString().slice(0, 10);
		await service.createItem(
			householdId,
		...
		await service.updateAutoExpiredGraceDays(householdId, 14, 'owner');

		expect(await service.getAutoExpiredGraceDays(householdId)).toBe(14);
		expect(await service.countAutoExpiredByLocation(householdId, 'fridge')).toBe(0);
		expect(await service.countActiveByLocation(householdId, 'fridge')).toBe(1);
	});
});

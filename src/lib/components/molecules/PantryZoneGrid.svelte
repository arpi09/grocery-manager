<script lang="ts">
	...
	let { zone, canConsume = false, onZoneOpen, onConsume }: Props = $props();
		...
		{#each zone.tiles as tile (tile.itemId)}
			<div role="listitem">
				<ProductTile
					...
					onConsume={() => handleConsume(tile.itemId)}
				/>
			</div>
		{/each}
...
<style>
	...
	@media (min-width: 480px) {
		.tile-grid {
			grid-template-columns: repeat(auto-fill, minmax(5.25rem, 6.25rem));
	...
	@media (min-width: 900px) {
		.tile-grid {
			grid-template-columns: repeat(auto-fill, minmax(5.5rem, 6.75rem));
		}
	}
</style>

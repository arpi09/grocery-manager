<script lang="ts">
	...
	const signupUrl = $derived(buildDelaSignupUrl($page.url.origin));
	const loginUrl = $derived(buildDelaLoginUrl(data.token));
	const tokenPrefix = $derived(tokenTelemetryPrefix(data.token));
	const itemCount = $derived(data.preview.items.length);
...
<main class="share-page">
	<div class="share-shell">
		<header class="share-header">
			...
			{#if itemCount > 0}
				<p class="item-count">{t('expiringShare.publicItemCount', { count: itemCount })}</p>
			{/if}
		...
		<div class="signup-cta">
			...
			<p class="signup-hint">{t('expiringShare.publicSignupHint')}</p>
			<a class="signup-login-link" href={loginUrl} onclick={handleLoginClick}>
				{t('expiringShare.publicLoginLink')}
			</a>
		</div>
	</div>
</main>

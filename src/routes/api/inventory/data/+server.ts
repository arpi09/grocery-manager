...
async function enrichAndSerialize(items: InventoryItem[], locals: App.Locals) {
	const enriched = await locals.productCatalogService.enrichInventoryItems(items);
	return enriched.map(serializeItem);
}
...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	if (sectionParam === 'active') {
		...
		if (searchQuery.length >= 2) {
			...
			return json({
				items: await enrichAndSerialize(items, locals),
				total: items.length
			});
		}

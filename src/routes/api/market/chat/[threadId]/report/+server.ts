...
import { marketChatReportSchema } from '$lib/validation/market-profile.schemas';
...
export const POST: RequestHandler = async ({ locals, params, request }) => {
	...
	const parsed = marketChatReportSchema.safeParse(body ?? {});
	if (!parsed.success) {
		return json(
	...
	const result = await marketChatService.reportThread({
		threadId: params.threadId,
		reporterUserId: auth.user.id,
		reason: parsed.data.reason,
		blockCounterpart: parsed.data.blockCounterpart ?? false
	});
	...
	return json({ ok: true, reportId: result.data.reportId, thread: result.data.thread });
};

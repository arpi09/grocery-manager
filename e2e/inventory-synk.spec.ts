...
test.describe('Inventory sync batch review', () => {
	...
	test('synk page loads with review or empty state', async ({ page }) => {
		...
		await expect(page.getByRole('heading', { name: /Fortfarande hemma|Still at home/i })).toBeVisible({
		...
		const emptyTitle = page.getByText(/Inget att synka|Nothing to sync/i);

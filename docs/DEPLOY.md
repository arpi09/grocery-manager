...
- [ ] **`FIREBASE_SERVICE_ACCOUNT`** (rekommenderat) eller **`FIREBASE_TOKEN`** satt i GitHub Secrets — se [Troubleshooting](#troubleshooting)
...
| Secret | Syfte |
|--------|-------|
| `FIREBASE_SERVICE_ACCOUNT` | **Rekommenderat.** JSON-nyckel för GCP service account med Firebase/App Hosting deploy-rättigheter. Används via `google-github-actions/auth` — undviker IAM 409 från `firebase login:ci`-token. Se [Troubleshooting — IAM 409](#troubleshooting) |
| `FIREBASE_TOKEN` | Fallback: `npx firebase login:ci` — fungerar men kan ge transient IAM 409 vid parallella deploys |
...
Utan `FIREBASE_SERVICE_ACCOUNT` **eller** `FIREBASE_TOKEN` körs quality + E2E ändå; deploy-jobbet skippar med tydlig loggrad.
...
1. Skapa GCP service account (t.ex. `github-deploy@home-pantry-4bee5.iam.gserviceaccount.com`) med roller: **Firebase App Hosting Admin**, **Service Account User**, **Secret Manager Secret Accessor** (eller bredare **Firebase Admin** för enklare setup).
2. Ladda ner JSON-nyckel → GitHub → **Settings → Secrets → Actions** → **`FIREBASE_SERVICE_ACCOUNT`** (hela JSON-filen).
3. Stäng av **App Hosting → GitHub auto-deploy** i Firebase Console om den fortfarande är på.
4. Kör deploy igen. Med service account behövs inte `FIREBASE_TOKEN`.

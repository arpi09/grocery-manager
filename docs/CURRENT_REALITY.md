...
| **Prod SHA** | `6ce2be61e` @ [28739479233](https://github.com/arpi09/grocery-manager/actions/runs/28739479233) (live, release 2026.7.5.2, deploy_tier=auto→fast, e2e critical grön; curl-smoke /, /guider, /login, /inkop = 200; prod `/sw.js` innehåller `cleanupOutdatedCaches` = nya loop-säkra SW live). **PWA SW reload-loop-fix** (#228). |
| **Master SHA** | `6ce2be61e` — PWA: service worker update-reload guardad mot oändlig loop (iOS Safari "laddar aldrig + varm telefon"). Föregående: declutter-batch #223 Lager + #224 Äta. |
